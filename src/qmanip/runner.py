...
from .experiments import ExperimentConfig, LegacyCommand
        ...
        if stage in {"aggregate", "all"}:
            if self.dry_run:
                for spec in experiment.aggregate_specs:
                    print(
                        "[aggregate] "
                        f"{experiment.workdir_path / spec.output_name} "
                        f"<- {spec.input_template}"
                    )
            else:
        ...
        for step in experiment.collect_steps:
            if step.per_seed:
                for seed in range(start, end):
                    seed_dir = experiment.workdir_path / str(seed)
