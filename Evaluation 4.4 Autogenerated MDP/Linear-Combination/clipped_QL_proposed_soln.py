...
def main():
    ...
    for avg in range(st, ed):
        ...
        for bf in range(1, 6, 2):
            ...
            # Save results
            pd.DataFrame([rewards]).to_csv(f'{avg}//ClippedQL_0_{bf}.csv')
            data.append((avg, f'0_{bf}', end_time - start_time))
    
    # Save all experiment data
    pd.DataFrame(data, columns=["Run", "Domain info", "ClippedQL"]).to_csv(f"Data_ClippedQL_{ed-1}.csv")
