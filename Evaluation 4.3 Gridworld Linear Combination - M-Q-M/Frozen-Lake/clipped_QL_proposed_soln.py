    ...
    for avg in range(init,till): #for avg in range(30):
        for nst in range(1, 5):
            ...
            rewards = clipped_q_learning(Q1,
                env, T1, 
                N_steps=24000,  # 4800*5 from original
                test_steps=8,
                gamma=0.9,
                ts=ts
            )
            ...
            # Save results
            pd.DataFrame([rewards]).to_csv(f'{avg}/ClippedQL_0_{nst}.csv')
            data.append((avg, f'0_{nst}', end_time - start_time))
    
    # Save all experiment data
    pd.DataFrame(data, columns=["Run", "Domain info", "ClippedQL"]).to_csv(f"Data_ClippedQL_{till-1}.csv")
