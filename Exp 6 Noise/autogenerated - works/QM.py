...
# Example usage
filename = "RA_idx.txt"
# st, ed = 0,30#read_and_increment(filename)
start_index, end_index = parse_seed_range("start: ", "end: ")
print("Seed range:", start_index, end_index)
for avg in range(start_index, end_index):
    ...
    for noise in [0.006,0.012,0.018,0.024,0.03]:
        ...
        if os.path.exists(file_path):
            ...
            # Create the target data by adding noise to the sum of R and R2
            r = R + R2 + np.random.uniform(-noise, noise, size=R.shape)
        ...
        end_time = time.time()
        pd.DataFrame(rewards_run).to_csv(f'{avg}//ours_{x_percentage}_{noise}_new.csv')
        w=100
        ...
        # break
pd.DataFrame(data, columns=['Run', 'Domain', '|S|', '|A|', 'Actions Pruned']).to_csv(f"Data_RA_{end_index-1}_new.csv")
