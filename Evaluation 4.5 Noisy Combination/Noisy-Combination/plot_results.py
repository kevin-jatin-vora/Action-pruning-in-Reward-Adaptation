...
df['DomainLabel'] = df['Domain'].apply(lambda x: f"{(float(x.split('_')[1]) / 1.2) * 100:.1f}%")
...
new_df['DomainLabel'] = new_df['Domain'].apply(lambda x: f"{(float(x.split('_')[1]) / 1.2) * 100:.1f}%")
...
# Reward plot setup
# ----------------------
reward_files = [f'ours_0_{v}.csv' for v in [0, 0.006, 0.012, 0.018, 0.024, 0.03]][1:]  # excluding 0
reward_files_new = [f.replace('.csv', '_new_new.csv') for f in reward_files]  # New version

params = {"w3": 10, "length": 170}
...
if os.path.exists(ql_file):
    axs[2].plot(x_axis, smoothed_ql_mean, label="QL", color='#ff7f0e', linestyle='--')
