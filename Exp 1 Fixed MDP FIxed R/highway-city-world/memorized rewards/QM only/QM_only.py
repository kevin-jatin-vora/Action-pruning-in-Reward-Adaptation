...
#     for a in range(A):
#         Qm[s,a]= max(o1[s,a], o2[s,a])-1.4
# np.save("memorized rewards//QM only//QM_only_LB_MR_init.npy", Qm)

# np.save("memorized rewards//QM only//QM_only_UB_MR_init.npy", q_p1 + q_p2 +1.4)

# np.save("memorized rewards//QM only//R1_memorized.npy", r1)

# np.save("memorized rewards//QM only//R2_memorized.npy", r2)
######################################################################################
...
for run in range(num_runs):
    Q = np.zeros((S,A)) #combined_Q.copy() #
...
end_time = time.time()
np.save("memorized rewards//QM_only_ours_learning.npy", rewards_run)
