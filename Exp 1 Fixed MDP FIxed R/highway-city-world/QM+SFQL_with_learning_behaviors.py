...
def episodic_q_learning(S, A, T1, R, goals):
    alpha = 0.1
    gamma = 0.9
    epsilon_initial = 1.0
...
q_p1, q_m1 = episodic_q_learning(S, A, T, R1, terminal_state)
# Compute Q-values
q_p2_old = compute_q_values(S, A, R2, T, gamma)
q_p2, q_m2 = episodic_q_learning(S, A, T, R2, terminal_state)
...
end_time = time.time()
np.save("QM+SFQL_ours_learning.npy", rewards_run)
