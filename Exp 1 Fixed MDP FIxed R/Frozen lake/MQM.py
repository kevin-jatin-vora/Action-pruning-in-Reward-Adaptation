...
A=4
data = []
start_index, end_index = parse_seed_range("enter start index: ", "enter end index: ")
for avg in range(start_index, end_index): #for avg in range(30):
...
pd.DataFrame(data, columns=['Run', 'Domain', '|S|', '|A|', 'Actions Pruned', 'QM']).to_csv(f"Data_RA_{end_index-1}.csv")
