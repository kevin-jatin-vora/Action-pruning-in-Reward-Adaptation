...
for avg in range(init,last):
    ...
    for nst in [1,2,4]:#range(1,5):
        ...
        ax.invert_yaxis()
        plt.savefig(f"{avg}//heatmap_DE_{x_percentage}_{number_stochastic_transitions}_new.png",bbox_inches = 'tight', dpi=1000)
        plt.show()
        # input()
        data.append((avg, f"Dollar-Euro_{x_percentage}_{number_stochastic_transitions}_new", S, A, S*A-sum([len(state_action[i]) for i in state_action.keys()]), end_time-start_time))
