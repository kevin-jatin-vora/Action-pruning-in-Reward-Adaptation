...
for avg in range(st,ed):
    ...
    for bf in range(1,6,2):
        ...
        data.append((avg, f"ours_{bf}_new", S, A, S*A-sum([len(state_action[i]) for i in state_action.keys()]), end_time-start_time))
        # break
pd.DataFrame(data, columns=['Run', 'Domain', '|S|', '|A|', 'Actions Pruned', 'QM']).to_csv(f"Data_RA_{ed-1}_new.csv")
